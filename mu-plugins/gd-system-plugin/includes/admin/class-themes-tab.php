...
final class Themes_Tab {
	...
	public function redirect() {
		...
		if ( 'theme-install.php' === $pagenow && ! sanitize_key( filter_input( INPUT_GET, 'browse' ) ) ) {
	...
	private function get_themes() {
		...
		foreach ( self::$recommended_themes as $key => $recommended_theme_slug ) {
			...
			$theme_data     = json_decode( wp_remote_retrieve_body( Plugin::remote_get( sprintf( 'https://api.wordpress.org/themes/info/1.1/?action=theme_information&request[slug]=%s', $recommended_theme_slug ) ) ), true );
