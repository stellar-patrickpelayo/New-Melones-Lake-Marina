...
final class Bundled_Plugins {
	...
	public function __construct() {
		...
		$bundled_plugins = [
			...
			// Must load before mcp-adapter.
			'wordpress/abilities-api/abilities-api.php' => $this->should_load_mcp_plugin( 'abilities-api/abilities-api.php' ),
			'wordpress/mcp-adapter/mcp-adapter.php' => $this->should_load_mcp_plugin( 'mcp-adapter/mcp-adapter.php' ),
			'gdcorp-wordpress/mcp-adapter-initializer/mcp-adapter-initializer.php' => $this->should_load_mcp_plugin( 'mcp-adapter-initializer/mcp-adapter-initializer.php' ),
			'gdcorp-wordpress/wp-site-designer-mu-plugins/wp-site-designer-mu-plugins.php' => $this->should_load_wp_site_designer_mu_plugins()
		];
	...
	/**
	 * Determine if the MCP plugin should be loaded.
	 *
	 * @param string $basename
	 *
	 * @return boolean
	 */
	public function should_load_mcp_plugin( $basename ) {
		...
		if ( is_readable( trailingslashit( WP_PLUGIN_DIR ) . $basename ) ) {
			return false;
		}

		// If environment is not prod, load the MCP plugins.
		if ( 'prod' !== Plugin::get_env() ) {
			return true;
		}
	...
	/**
	 * Determine if the WP Site Designer MU Plugins should be loaded.
	 *
	 * @param string $basename
	 *
	 * @return boolean
	 */
	public function should_load_wp_site_designer_mu_plugins() {
		// If environment is not prod, load the plugin.
		if ( 'prod' !== Plugin::get_env() ) {
			return true;
		}

		// Check if the feature flag is enabled.
		if ( ! $GLOBALS['wpaas_feature_flag']->get_feature_flag_value( 'wp_site_designer_plugins', false ) ) {
			return false;
		}
	...
	private function maybe_load_plugin( $basename ) {
		...
		$mcp_plugins = array(
			...
			'gdcorp-wordpress/mcp-adapter-initializer/mcp-adapter-initializer.php',
		);

		// Load the MCP plugins a bit later to ensure proper load order.
		// Abilities API must always load first.
		if ( in_array( $basename, $mcp_plugins, true ) ) {

			add_action( 'plugins_loaded', function() use ( $basename, $path ) {
				require_once $path;
			}, PHP_INT_MAX );
