...
final class API implements API_Interface {
		...
		while ( $retries <= $max_retries ) {
			...
			switch ( $http_verb ) {
				...
				case 'GET':
					...
					$response = Plugin::remote_get( $url, $http_args );
					break;
				...
				case 'POST':
					...
					$response = Plugin::remote_post( $url, $http_args );

					break;
				case "DELETE":
					$http_args['body'] = (bool) apply_filters( 'wpaas_api_http_post_body_json', false ) ? json_encode( $method_args, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE ) : $method_args;
					$response          = Plugin::remote_request( $url, array_merge( $http_args, [
						'method' => 'DELETE'
					] ) );
					break;
    ...
    public function get_trusted_site_sso_url() {

        add_filter( 'wpaas_api_url', [ $this, 'wp_public_api_url' ] );
        add_filter( 'wpaas_api_http_args', [ $this, 'v3_api_http_args' ] );

        $response = $this->call( $this->wp_public_api_accountuid( "v3-proxy/%s/trustedsite" ) );
    ...
    public function show_nps_form($npsFormId) {
        add_filter( 'wpaas_api_url', [ $this, 'wp_public_api_url' ] );
        add_filter( 'wpaas_api_http_args', [ $this, 'v3_api_http_args' ] );

        try {
            $accountUid = defined( 'GD_ACCOUNT_UID' ) ? GD_ACCOUNT_UID : '';
            $response   = $this->call( 'feedback-api-proxy/' . $accountUid . '/' . rawurlencode($npsFormId) . '/show');
            $body = json_decode( wp_remote_retrieve_body( $response ), true );
            return $body['show'];
        } catch ( WP_Error $e ) {
            return null;
    ...
    public function get_nps_form_configuration($nps_form_id) {
        add_filter( 'wpaas_api_url', [ $this, 'wp_public_api_url' ] );
        add_filter( 'wpaas_api_http_args', [ $this, 'v3_api_http_args' ] );

        try {
            $accountUid = defined( 'GD_ACCOUNT_UID' ) ? GD_ACCOUNT_UID : '';
            $response   = $this->call( 'feedback-api-proxy/' . $accountUid . '/' . rawurlencode($nps_form_id) . '/form-configuration');
    ...
    public function nps_form_submit($form_data, $nps_form_id) {
        add_filter( 'wpaas_api_url', [ $this, 'wp_public_api_url' ] );
        add_filter( 'wpaas_api_http_args', [ $this, 'v3_api_http_args' ] );
        add_filter( 'wpaas_api_http_post_body_json', '__return_true' );
        ...
        try {
            ...
            $data = [
                ...
                'feedbackId' => $nps_form_id,
                'data' => $form_data
            ];

            $this->call( 'feedback-api-proxy/nps-submit', $data, 'POST' );
        } catch ( WP_Error $e ) {
            return null;
        }
        $nps_option_name = 'wpaas_nps_form_%s';

        $option_name = sprintf( $nps_option_name, GD_ACCOUNT_UID );
    ...
    public function nps_form_dismiss($nps_form_id) {
        add_filter( 'wpaas_api_url', [ $this, 'wp_public_api_url' ] );
        add_filter( 'wpaas_api_http_args', [ $this, 'v3_api_http_args' ] );
        add_filter( 'wpaas_api_http_post_body_json', '__return_true' );
        ...
        try {
            ...
            $response = $this->call( 'feedback-api-proxy/nps/dismiss', $data, 'POST' );
    ...
    public function feedback_form_submit ($form_data, $feedback_form_id) {
        add_filter( 'wpaas_api_url', [ $this, 'wp_public_api_url' ] );
        add_filter( 'wpaas_api_http_args', [ $this, 'v3_api_http_args' ] );
        add_filter( 'wpaas_api_http_post_body_json', '__return_true' );
        ...
        try {
            ...
            $data = [
                'accountUid' => $accountUid,
                'feedbackId' => $feedback_form_id,
                'data' => $form_data
            ];

            $this->call( 'feedback-api-proxy/submit', $data, 'POST' );
        } catch ( WP_Error $e ) {
            return null;
        }
